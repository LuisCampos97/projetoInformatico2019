...
export default {
  ...
  mounted() {
    axios.get('api/ctc/getPropostas/'+this.$store.state.user.id).then(response => {
      this.numeroPropostas = response.data.length;
    });

    axios.get('api/ctc/getPropostasPorTipoParecer/'+this.$store.state.user.id).then(response => {
